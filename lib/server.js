...
function staticServer(options) {
    ...
    if (options.reload) {
        io(server).on("connection", (client) => {
            ...
            client.on('dependencies', deps => {
                deps = fetchd(JSON.parse(deps))
                console.log(deps);
            })

            chokidar.watch(options.public, { interval: 1500 }).on('change', (fpath) => {

                console.log(fpath);

                //console.log(relative(options.public, fpath));
                //client.emit(wevent, relative(options.public, fpath))
            })
        });
    }
...
function fetchd(dep) {
    function tri(d) {
        if (d.deps.length == 0) return d.file
        return [d.file, ...d.deps.map(dp => tri(dp))].join(',')
    }
    return dep.map(d => tri(d)).join(',').split(',')
}
