...
function enableReaload(res, file, reload, root) {
  ...
  if (reload) {
    ...
    script = `<script >
            ...
            ${readFileSync(resolve(__dirname, "reload-client.js"), {
              encoding: "utf-8",
            })}
        </script>`;
...
function staticServer(options) {
  ...
  async function onConnection(req, res) {
    ...
    res.end();
    log(
      `${req.method.toLocaleUpperCase()} ${pathname} 200 ${STATUS_CODES["200"]}`
    );
  }
