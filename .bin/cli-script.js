...
async function inquireHost(options, rl, callback) {
  if (!isValidHost(options)) {
    rl.question(
      options.host + " is invalid, enter a new one (127.0.0.1)",
      (host) => {
        if (host == "") {
    ...
    rl.question(
      "are you sure you want to bind on " + options.host + " ? [Y/n](Y) : ",
      (answer) => {
...
function isValidHost(options) {
  if (
    ...
    !options.host.match(/[0-9]{1,3}(?:\.[0-9]{1,3}){3}/g)
  ) {
    return false;
  } else if (options.host.match(/[0-9]{1,3}(?:\.[0-9]{1,3}){3}/g)) {
    let hasErr = false;
    options.host.split(".").map((h) => {
...
inquireHost(options, rl, async (options) => {
  ...
  if (port !== options.port)
    console.log(
      `port ${options.port} already in use , ${port} will be use instead`
    );
  ...
  server.listen(port, host, () => {
    ...
    try {
      ...
      console.log(
        "\r\n unable to open the default browser, you should open it by your self ."
      );
    }
  });
});
